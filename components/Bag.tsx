...
const Bag: React.FC<BagProps> = ({ bagToggle, bagToggles }) => {
  const [cartSetting, setCartSetting] = useState<Product[]>([]);
  ...
  const groupedProducts = cartSetting.reduce((acc: { [key: string]: { product: Product, quantity: number } }, product) => {
    if (acc[product._id]) {
